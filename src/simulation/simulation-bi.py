...
#   - Only difficulty
#   - Removed grasp_attempts
#   - Added outcome: success / missed_target / fell_off_table
# ============================================================
PARTICIPANT_ID = "P01"
LOG_FILE = f"results_{PARTICIPANT_ID}_bimanual.csv"

SUCCESS_THRESH = 0.05          # 2D (XY) success radius at release
FLOOR_Z_THRESH = 0.06          # below this => "fell_off_table"

N_REPS_PER_LEVEL = 2
LEVELS = ["easy", "medium", "hard"]

# Only cube moves (difficulty), target fixed
CUBE_POS_BY_LEVEL = {
...
def reset_trial_state():
    global trial_start, cube_attached, cid, drops, time_to_grasp
    trial_start = time.time()
    cube_attached = False
    cid = None
    drops = 0
...
def create_fixed_constraint_preserve_pose(parent_body, parent_link, child_body):
    """Fixed constraint preserving current relative pose (prevents cube snapping)."""
...
def log_and_advance(outcome: str, placement_error: float, cube_pos_xyz):
    ...
    placement_error: 2D error (XY) or np.nan
    """
    global trial
...
# ============================================================
# BIMANUAL RULE
# Left hand controls BASE + SHOULDER
# Right hand controls WRIST + ELBOW
# ============================================================
...
try:
    while True:
        ...
        # ---------- AUTO FAIL if cube fell off table (even mid-trial) ----------
        ...
        # release -> END TRIAL always (success OR missed_target)
        if pinch_open_event and cube_attached:
            ...
            # if missed_target: count as drop (kept from your code)
            if outcome == "missed_target":
                drops += 1
                # wake physics so it falls naturally
        ...
        # ---------- HUD ----------
        cv2.putText(frame,
                    f"TRIAL: {trial+1}/{N_TRIALS} | difficulty={current_difficulty} | D={current_D:.2f}",
                    (10, h - 95),
