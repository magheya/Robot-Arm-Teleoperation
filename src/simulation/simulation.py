...
if not os.path.exists(LOG_FILE):
    with open(LOG_FILE, "w", newline="") as f:
        csv.writer(f).writerow([
            "participant","trial","factor","difficulty","D",
            "time_total","time_to_grasp","placement_error","success",
            "grasp_attempts","drops",
            "cube_x","cube_y","cube_z","target_x","target_y","target_z"
        ])

LEVELS = ["easy", "medium", "hard"]

# Only cube moves (grasp difficulty); target fixed
CUBE_POS_BY_LEVEL = {
...
try:
    while shared.is_running and trial_idx < len(TRIAL_PLAN):
        ...
        # Gripper projection only if not attached
        ...
        if display_frame is not None:
            ...
            cv2.putText(display_frame,
                        f"TRIAL: {trial_idx+1}/{len(TRIAL_PLAN)} | {current_factor} ({current_difficulty})",
            ...
            cv2.putText(display_frame,
                        f"Attempts: {grasp_attempts} | Drops: {drops} | D={current_D:.3f}",
